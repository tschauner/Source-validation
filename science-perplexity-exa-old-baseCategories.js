// science-perplexity-exa-optimized-v3.js — Multi-Tier Validation with Falsification Test
...
const USE_GPT_SANITY_CHECK = true; // Set to false to disable GPT layer
...
let METRICS = {
  ...
  validation: {
    ...
    tier3_no: 0,
    tier3_unclear: 0,
    tier3_year_corrected: 0,
    tier4_success: 0,
    tier4_fail: 0,
    tier4_falsification_failed: 0,
    tier4_title_matches: 0,
    tier5_success: 0,
    tier5_fail: 0
  }
};
...
// ---------- TIER 3: Perplexity Validator ----------
async function validateWithPerplexity(event, monthName, day) {
  console.log(`\n      🔍 TIER 3: Perplexity Date Validator`);
  console.log(`         Event: ${event.title}`);
  console.log(`         Date: ${monthName} ${parseInt(day)}, ${event.year}`);
...
Be strict: Only answer YES if you can confirm the exact date with reliable sources.`;
  ...
  try {
    ...
    // Update metrics
    if (verdict === 'YES') {
      METRICS.validation.tier3_yes++;
      console.log(`         ✅✅ PERPLEXITY CONFIRMS DATE!`);
    } else if (verdict === 'NO') {
      METRICS.validation.tier3_no++;
      console.log(`         ❌ PERPLEXITY REJECTS DATE`);
    } else {
      METRICS.validation.tier3_unclear++;
...
Return only the rewritten context:`;
    ...
    try {
      ...
      METRICS.validation.tier3_year_corrected++;
...
// ---------- TIER 4: Enhanced Differential with Natural Language + Title Analysis ----------
async function validateWithDifferentialQuery(event, monthName, day, optimizedKeywords = null) {
  console.log(`\n      🔍 TIER 4: Enhanced Differential + Falsification`);
  console.log(`         Event: ${event.title}`);
  ...
  try {
    ...
    METRICS.validation.tier4_title_matches += titleMatchesB;
    ...
    if (retentionC > retentionB) {
      ...
      METRICS.validation.tier4_falsification_failed++;
...
async function validateEventReality(event, monthName, day) {
  console.log(`\n   🛡️ === MULTI-TIER REALITY VALIDATION ===`);
  
  // PHASE 1: FREE WIKIPEDIA CHECKS
  ...
  // TIER 1: Wikipedia Article + QID (free check!)
  if (event.qid) {
  ...
  // PHASE 2: PAID VALIDATION ($ - optimized order)
  
  // TIER 2: GPT Sanity Check + Keyword Optimization (cheap - $0.00001)
  let keywords = event.keywords;
  if (USE_GPT_SANITY_CHECK) {
    const tier2 = await validateWithGPTSanityCheck(event, monthName, day);
    if (tier2.validated === false) {
      // GPT says IMPLAUSIBLE - reject immediately
      return { valid: false, method: 'tier2-gpt-sanity', reason: tier2.reason };
    }
    // Use GPT-optimized keywords
    keywords = tier2.keywords || event.keywords;
  }
  
  // TIER 3: Perplexity Validator (moderate - $0.003, saves $0.006 on Differential!)
  const tier3 = await validateWithPerplexity(event, monthName, day);
  
  if (tier3.verdict === 'NO') {
    // Check if we can auto-correct the year (month+day correct, only year wrong)
    if (tier3.actualDate) {
      const correction = await correctEventYear(event, tier3.actualDate, tier3.reason, monthName, day);
      
      if (correction && correction.corrected) {
        console.log(`         ✅✅ YEAR AUTO-CORRECTED: ${correction.oldYear} → ${correction.newYear}`);
        return { valid: true, method: 'tier3-perplexity-year-corrected', reason: 'year-auto-corrected' };
      }
    }
    
    // Cannot correct - full date mismatch
    return { valid: false, method: 'tier3-perplexity', reason: 'perplexity-rejected' };
  }
  
  if (tier3.verdict === 'YES' && tier3.confidence === 'HIGH') {
    // High confidence YES - skip Differential!
    return { valid: true, method: 'tier3-perplexity', reason: 'perplexity-confirmed-high' };
  }
  
  // TIER 4: Enhanced Differential Query (expensive - $0.006)
  // Run if: Perplexity is UNCLEAR or YES with LOW/MEDIUM confidence
  console.log(`\n      ⚠️ Perplexity ${tier3.verdict} (${tier3.confidence}) - running Differential...`);
  
  const tier4 = await validateWithDifferentialQuery(event, monthName, day, keywords);
  
  if (tier4.validated === false) {
    return { valid: false, method: 'tier4-differential', reason: tier4.reason };
  }
  
  if (tier4.validated === true) {
    return { valid: true, method: 'tier4-differential', reason: tier4.reason };
  }
  
  // TIER 5: Content Verification (very expensive - $0.008, only ~5% of events)
  if (tier4.validated === null) {
    console.log(`\n      ⚠️ Differential uncertain - final content check...`);
    const tier5 = await validateWithSourceContent(event, monthName, day, tier4.results);
    if (tier5.validated) {
      return { valid: true, method: 'tier5-content-verified', reason: tier5.reason };
    } else {
      return { valid: false, method: 'tier5-content-verification', reason: tier5.reason };
    }
  }
  
  // Failed all validation
  return { valid: false, method: 'all-tiers-failed', reason: tier4.reason };
}
